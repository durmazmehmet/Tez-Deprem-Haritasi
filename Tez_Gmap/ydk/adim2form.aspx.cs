    ...
    public partial class WebForm1 : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string[] td_1 = _adimiki[8].ToString().Split(bolbeni, StringSplitOptions.RemoveEmptyEntries);
            ...
            //GMap2.setCenter(new GLatLng(38.92, 35.26), hZoom);
            ...
            else
            {
                ...
                c2.eqinPKoord.AddRange(GetPointsIntoPolygon(g2.polyKoord, c2.eqKoord));
                ...
                for (int i = v2.vBasla; i < c2._magnitud.Length - 1; i++)
                {
                    ...
                    eq.icon = eq_icon;
                    GMarker eq_isrt = new GMarker(c2.eqKoord[i], eq);
                ...
                for (int i = 1; i < _mCopy.Count; i++)
                    kumulatif.Add((_magCopy.FindAll(delegate(double p) { return p.Equals(_mCopy[i]); }).Count) + kumulatif[i - 1]);
                ...
                if (Session["kesme"] != null)
                {
                    ...
                    kumulatifLQ.Add((_magCopy.FindAll(delegate(double p) { return p == _mCopyLQ[0]; })).Count);
                    for (int i = 1; i < _mCopyLQ.Count; i++)
                        kumulatifLQ.Add((_magCopy.FindAll(delegate(double p) { return p.Equals(_mCopyLQ[i]); }).Count) + kumulatifLQ[i - 1]);
        ...
        public List<GLatLng> GetPointsIntoPolygon(List<GLatLng> PoliKoords, List<GLatLng> Koords)
        {
            ...
            foreach (GLatLng Koord in Koords)
            {
                ...
                //Eğer nokta poligon içinde ise valor çift sayıdır
                if ((valor % 2) != 0)
